...
export function CornerJourney() {
  ...
  return <section className="journey-scene" id="corner-journey" data-reading-position data-guide-chapter aria-label="从路面细节走进旋转木马">
    <div className="journey-viewport">
      ...
      <div className="journey-rail">
        ...
        <div className="journey-detail-group" aria-label="旋转木马八张交错影像">{journeyPhotos.map(({ photo, title, english, text, x, y, w, h, position }, index) => <figure className={`journey-detail journey-detail-${index}`} style={{ '--photo-x': `${x}vw`, '--photo-y': `${y}%`, '--photo-w': `${w}vw`, '--photo-h': `${h}svh`, '--photo-position': position }} key={title} data-journey-panel>
          <div className="journey-detail-image"><img src={photo.src} srcSet={photo.srcSet} sizes={`(min-width: 1000px) ${w}vw, (min-width: 600px) 40vw, 78vw`} alt={`${title}：${text}`} loading="lazy" fetchPriority="low" width={photo.width} height={photo.height} /></div>
        ...
        <div className="journey-finale" data-journey-panel>
          <img className="journey-finale-photo" src={karussell} alt="旋转木马全景，混凝土内槽与外侧沥青组成回头弯" width="1600" height="899" loading="lazy" fetchPriority="low" />
    ...
    <div className="journey-static-trail" aria-hidden="true">{journeyPhotos.map(({ photo, title, english, text, position }, index) => <figure className="journey-trail-item" key={title}>
      <img src={photo.src} srcSet={photo.srcSet} sizes="(min-width: 1000px) 38vw, 78vw" alt="" loading="lazy" fetchPriority="low" width={photo.width} height={photo.height} style={{ objectPosition: position }} />
...
export function MachineChapter() {
  ...
  return <section className="machine-chapter" id="machines" data-reading-position data-guide-chapter aria-labelledby="machine-title">
    ...
    <div className="machine-contact-sheet">
      <figure className="machine-road" data-drift><Link to="/brands/porsche/911-gt3-rs" data-cursor="读车"><img src={gt3.src} alt="车展上的 Porsche 911 GT3 RS" width="1600" height="1156" loading="lazy" fetchPriority="low" /><span>01 / ROAD CAR <ArrowUpRight size={18} /></span></Link><figcaption>911 GT3 RS / 车展实拍</figcaption><ImageCredit href={gt3.source}>{gt3.author} · {gt3.license} · 裁切</ImageCredit></figure>
      <div className="machine-note"><span>FORM FOLLOWS<br /><em>the circuit.</em></span><p>尾翼、轮胎、车身。<br />看懂它们，再读计时器。</p></div>
      <figure className="machine-detail" data-drift><div><img src={porscheSide.src} srcSet={porscheSide.srcSet} sizes="(min-width: 760px) 33vw, 55vw" alt="Rennsport Reunion VI 活动现场，919 Hybrid Evo 的低矮车身与尾翼" width={porscheSide.width} height={porscheSide.height} loading="lazy" fetchPriority="low" /></div><figcaption>02 / THE SILHOUETTE<br /><span>低矮座舱与高耸尾翼 · 2018 活动现场</span></figcaption><ImageCredit href={porscheSide.source}>{porscheSide.author} · {porscheSide.license} · 展示裁切</ImageCredit></figure>
    </div>
...
export function PhotoAtlas() {
  return <details className="story-photo-index">
    ...
    <div className="story-photo-grid">{trailPhotos.map(photo => <figure key={photo.thumb}>
      <a href={photo.source} target="_blank" rel="noreferrer"><img src={photo.thumb} alt={photo.title} loading="lazy" fetchPriority="low" width="320" height="220" /><strong>{photo.title} <ArrowUpRight size={12} /></strong></a>
